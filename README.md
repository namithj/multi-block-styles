...
1. Upload the `multi-block-styles` folder to `/wp-content/plugins/`.
...
Yes. Selecting a single style works the same as before — the only difference is that you can now select additional styles without deselecting the first.
...
No. Block styles are stored as CSS classes in the block's `className` attribute. Multiple `is-style-*` classes have always been valid HTML — WordPress just didn't provide a UI to select more than one.
